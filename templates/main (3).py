...
def predict_strategy(guaranteed_numbers, bonus1, bonus2, historical_draws):
    """
    ...
    # 6. Sort all numbers by their final score and select the top 4
...
def generate_live_prediction(historical_draws):
    ...
    if not historical_draws:
        logging.warning("No historical data to generate a live prediction.")
        return None
    ...
    # 2. Generate the final prediction using the new strategy
    predicted_mains = predict_strategy(guaranteed_numbers, latest_actual_bonus1, latest_actual_bonus2, historical_draws)
...
def backtest_strategy(draws_data):
    ...
    for i in range(len(draws_data) - 26):
        ...
        # CORRECTED AND VERIFIED: This now correctly uses only true past data for the prediction model
        ...
        # Generate prediction using the new strategy
        predicted_mains = predict_strategy(guaranteed_numbers, current_draw_b1, current_draw_b2, historical_data)
