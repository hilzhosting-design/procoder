...
from google.cloud.firestore_v1.field_path import FieldPat
...
def force_prediction_update():
    try:
        # Pass force_run=True to bypass the time check
        scrape_and_process_draws_job(force_run=True)
        flash("Forced scrape and prediction update initiated successfully!", "success")
    except Exception as e:
...
def scrape_and_process_draws_job(force_run=False):
    logging.info(f"--- JOB START: Scrape and Process Draws (Forced: {force_run}) ---")
    ...
    if force_run:
        ...
        if now.hour >= 19:
            ...
            logging.info("--- Scheduled run: Time to fetch Lunchtime results. ---")
            draws_to_fetch.append((UK_49S_LUNCHTIME_URL, 'Lunchtime'))
        else:
    ...
    all_scraped_draws = []
    for url, draw_type in draws_to_fetch:
