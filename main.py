...
def save_public_prediction(prediction_data, mode='live'):
    """Save prediction to Firestore only if in live mode."""
    if mode != 'live':
        logging.debug('Backtest mode: skipping Firestore save for public prediction.')
        return
    get_public_prediction_doc_ref().set(prediction_data)
...
def get_strategy_insights_doc_ref(date=None):
    """
    Fetches a reference to the strategy insights document.
    - For live predictions (date=None), it gets the 'latest' insights.
    - For backtesting (date provided), it gets the insights for that specific date.
    """
    app_id = get_app_id_for_firestore()
    collection_ref = db.collection('artifacts').document(app_id).collection('public_settings')
    if date:
        doc_id = f"strategy_insights_{date.strftime('%Y-%m-%d')}"
        return collection_ref.document(doc_id)
    else:
        return collection_ref.document('strategy_insights')
...
# --- FOCUSED SCORING Prediction Strategy v7 ---

def super_hybrid_pool(bonus):
    """Generates a pool of candidate numbers based on the super hybrid strategy."""
    mirror = 50 - bonus
    pool = list(set([bonus, mirror, bonus + 1, bonus - 1, bonus + 5,bonus - 5,bonus + 10, bonus - 10, bonus + 17, bonus - 17, bonus + 11, bonus - 11]))
    return [n for n in pool if 1 <= n <= 50]

def get_main_number_frequencies(historical_draws):
    """Calculates the frequency of each main number from historical data."""
    all_main_numbers = [num for _, mains, _, _, _ in historical_draws for num in mains]
    return Counter(all_main_numbers)

def get_hot_numbers(historical_draws, window=15):
    """Identifies numbers that have appeared frequently in a recent window."""
    if not historical_draws or len(historical_draws) < window:
        return []
...
def predict_strategy(guaranteed_numbers, bonus1, bonus2, historical_draws, target_date=None):
    """
    Generates a 4-number prediction using an advanced statistical scoring model.
    """
    scores = defaultdict(int)

    # 1. Assign a massive score to guaranteed numbers to ensure their selection
    for num in guaranteed_numbers:
        scores[num] += 1000

    # 2. Assign score boost from hybrid pools of BOTH bonus balls
    hybrid_pool = set(super_hybrid_pool(bonus1) + super_hybrid_pool(bonus2))
    for num in hybrid_pool:
        scores[num] += 75

    # 3. Assign score boost for "hot" numbers from the last 15 draws
    hot_numbers = get_hot_numbers(historical_draws, window=15)
    for num in hot_numbers:
        scores[num] += 60

    # 4. Assign score boost for "cold" numbers (due to appear)
    cold_numbers = get_cold_numbers(historical_draws, window=25)
    for num in cold_numbers:
        scores[num] += 40

    # 5. Assign a score based on overall historical frequency
    frequencies = get_main_number_frequencies(historical_draws)
    for num, freq in frequencies.items():
        scores[num] += freq * 3

    # 6. Assign score boost for historically successful bonuses
    try:
        # Fetch insights for the specific target_date if provided (for backtesting),
        # otherwise fetch the latest insights (for live predictions).
        insights_doc = get_strategy_insights_doc_ref(date=target_date).get()
        if not insights_doc.exists and target_date:
            # Fallback for backtesting: if no specific insights for that day, use the latest.
            insights_doc = get_strategy_insights_doc_ref().get()

        if insights_doc.exists:
            successful_bonuses = insights_doc.to_dict().get('bonuses_with_3_hits', [])
            for num in successful_bonuses:
                scores[num] += 50 # Add a significant boost
            logging.debug(f"Applied score boost for {len(successful_bonuses)} pre-computed successful bonuses.")
    except Exception as e:
        logging.error(f"Could not fetch or apply strategy insights: {e}")

    # 7. Sort all numbers by their final score and select the top 4
    sorted_candidates = sorted(scores.keys(), key=lambda num: scores[num], reverse=True)
    ...
    # Fallback: if somehow we still don't have 4, fill with randoms
    while len(final_prediction) < 4:
...
def generate_live_prediction(historical_draws):
    """Generates a live prediction for the next draw using the new model."""
    logging.info("--- Generating Live Prediction with Focused Scoring Model v7 ---")
    if not historical_draws or len(historical_draws) < 2:
        logging.warning("Not enough historical data to generate a live prediction.")
        return None

    latest_actual_draw = historical_draws[0]
    history_for_prediction = historical_draws[1:]
    
    latest_actual_mains = latest_actual_draw[1]
    latest_actual_bonus1 = latest_actual_draw[2]
    latest_actual_bonus2 = latest_actual_draw[3]
    
    # 1. Get the guaranteed numbers from the 4th main number
    fourth_main_num = latest_actual_mains[3]
    guaranteed_numbers = [n for n in [fourth_main_num + 1, fourth_main_num - 1] if 1 <= n <= 50]
    guaranteed_numbers = list(set(guaranteed_numbers))
    logging.info(f"Derived guaranteed numbers: {guaranteed_numbers} from 4th main number {fourth_main_num}")

    # 2. Generate the final prediction using the new strategy with the corrected history
    # For live prediction, we don't pass a date, so it uses the latest insights.
    predicted_mains = predict_strategy(guaranteed_numbers, latest_actual_bonus1, latest_actual_bonus2, history_for_prediction)
    
    logging.info(f"Generated live prediction: {predicted_mains}")
    return {'prediction': predicted_mains, 'strategy_used': 'focused_scoring_model_v7'}

def backtest_strategy(draws_data):
    """Performs backtesting on the given draws data using the new model."""
    logging.debug(f"backtest_strategy received {len(draws_data)} draws for processing.")
    results = []
    # Include latest live prediction in backtest results
    try:
        live_doc = get_public_prediction_doc_ref().get()
        if live_doc.exists:
            live_data = live_doc.to_dict()
            target_time = normalize_to_harare_time(live_data.get('target_draw_time'))
            # Find actual draw for the target time
            actual_draw = next(
                (d for d in draws_data if normalize_to_harare_time(d[0]) == target_time),
                None
            )
            if actual_draw:
                hits = len(set(live_data['prediction']) & set(actual_draw[1]))
                results.append({
                    'target_draw_time': target_time,
                    'prediction': live_data['prediction'],
                    'actual_mains': actual_draw[1],
                    'hits': hits,
                    'note': 'LIVE Prediction'
                })
    except Exception as e:
        logging.error(f'Error adding live prediction to backtest: {e}')

    if len(draws_data) < 27: # Need at least 25 for cold numbers + 2 for current/target
    ...
    for i in range(len(draws_data) - 26):
        current_draw_data = draws_data[i]
        target_actual_draw = draws_data[i + 1]
        historical_data = draws_data[i + 2:]

        current_draw_mains = current_draw_data[1]
        current_draw_b1 = current_draw_data[2]
        current_draw_b2 = current_draw_data[3]
        
        target_timestamp, target_mains, _, _, target_draw_type = target_actual_draw
        target_date_for_insights = target_timestamp.date()
        
        # --- Backtest adaptation of the new strategy ---
        fourth_main_num = current_draw_mains[3]
        guaranteed_numbers = [n for n in [fourth_main_num + 1, fourth_main_num - 1] if 1 <= n <= 50]
        guaranteed_numbers = list(set(guaranteed_numbers))

        # Generate prediction using the new strategy, passing the specific date for insights
        predicted_mains = predict_strategy(guaranteed_numbers, current_draw_b1, current_draw_b2, historical_data, target_date=target_date_for_insights)
        ...
        results.append({
            ...
            'strategy_used': 'focused_scoring_model_v7',
            'bonus': [current_draw_b1, current_draw_b2],
...
def scrape_and_process_draws_job():
    ...
    if live_prediction_result:
        ...
        try:
            ...
            save_public_prediction(prediction_data, mode='live')
...
def precompute_successful_bonuses_job():
    """
    Pre-computes successful bonuses and saves them to a dated document for backtesting
    and a 'latest' document for live predictions.
    """
    ...
    if len(historical_draws) < 20:
        logging.info("Not enough historical draws to pre-compute bonuses. Skipping.")
        return
    ...
    for result in backtest_results:
        if result.get('hits', 0) >= 3:
            bonuses_used = result.get('bonus', [])
            for b in bonuses_used:
                successful_bonuses.add(b)
    ...
    if successful_bonuses:
        ...
        # Save to the "latest" document for live predictions
        get_strategy_insights_doc_ref().set(insights_data)
        
        # Save to a dated document for historical backtesting consistency
        today_date = datetime.now(harare_tz).date()
        get_strategy_insights_doc_ref(date=today_date).set(insights_data)

        logging.info(f"Stored {len(successful_bonuses)} successful bonuses for {today_date.strftime('%Y-%m-%d')}.")
    else:
        logging.info("No new successful bonuses found to store.")
...
scheduler.add_job(precompute_successful_bonuses_job, 'cron', hour='12', minute='54', id='precompute_bonuses_job', replace_existing=True)
...
# --- Start Scheduler ---
# This block ensures the scheduler starts when the app is loaded by a WSGI server (like Gunicorn).
# It also registers a function to shut down the scheduler gracefully when the app exits.
try:
...
# The app object is now ready to be run by a WSGI server in production.
# For local development, you would typically run this file directly.
# Example for local run: `python your_app_file.py`
# The WSGI server (e.g., Gunicorn) will be specified in your deployment configuration (e.g., Procfile).
# Example Procfile: `web: gunicorn main:app`
